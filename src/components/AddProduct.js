...
function AddProduct(props) {
  ...
  // useStates for the input values, errors in the inputs and submit of a new product respectively
  const [formValues, setFormValues] = useState(initialValues);
  const [formErrors, setFormErrors] = useState({});
  const [isSubmit, setIsSubmit] = useState(false);

  // Function for submition of new product
  ...
  // useEffect that checks input information for new product is correct and updates subtotal value of the current order
  useEffect(() => {
    if (Object.keys(formErrors).length === 0 && isSubmit) {
      ...
      setIsSubmit(false);
      props.setTotal(
        props.subTotal + parseInt(totals.tax) + parseInt(totals.shipping)
      );
    }
  });
  ...
  const validate = (values) => {
    ...
    if (!values.quantity) {
      errors.quantity =
        "Please enter a number of products to add to the order!";
        ...
        <Card className="p-2">
          ...
          <Form onSubmit={handleSubmit}>
            ...
            >
              ...
              <Form.Control
                type=""
                placeholder="How many do you want to add to the order?"
              />
              <Form.Text id="nameAlert" className="text-danger fs-6">
                {formErrors.quantity}
              </Form.Text>
